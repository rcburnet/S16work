...
####
# Improved local_sky_subtraction.py script that will look into combined OB data cubes found in /home/rburnet/reflex/project_data/OBs_combined/sky_subtracted/ , do a local sky subtraction, calculate magnitude of target, and plot the IZ and YJ spectrum of each target as detailed in the email sent by Dr. Balogh on June 15. Note: Plotted spectra is not total flux, but median flux for each wavelength slice. Sky flux is total flux from correspond sky arm sky frame combined OBs. Magnitude is calculated by taking the sum of the aperture flux at each wavelength slice and choosing the median of all those wavelength slices as the flux and then calculating mag = -2.5 * log(flux/0.1*F0)
####
...
for root,dirnames,filenames in os.walk('/home/rburnet/reflex/project_data/OBs_combined/cubes_from_raw_data/combined_sky/'):
        for filename in fnmatch.filter(filenames, 'COMBINE_SCI_RECONSTRUCTED_ARM*'):
...
for i in range(len(hdulist)):
    if not False:
        ...
        for k in range(len(hdulist_sky)):
            if 'CL0034_YJ' in hdulist[i] and 'CL0034_YJ' in hdulist_sky[k]:
                if '_' in hdulist_sky[k][119:121]:
                    if int(hdulist_sky[k][119:120]) == origarm:
                        hdulist1_sky = fits.open(hdulist_sky[k])
                else:
                    if int(hdulist_sky[k][119:121]) == origarm:
                        hdulist1_sky = fits.open(hdulist_sky[k])
            if 'CL0034_IZ' in hdulist[i] and 'CL0034_IZ' in hdulist_sky[k]:
                if '_' in hdulist_sky[k][119:121]:
                    if int(hdulist_sky[k][119:120]) == origarm:
                        hdulist1_sky = fits.open(hdulist_sky[k])
                else:
                    if int(hdulist_sky[k][119:121]) == origarm:
                        hdulist1_sky = fits.open(hdulist_sky[k])
            if 'CL0036_YJ' in hdulist[i] and 'CL0036_YJ' in hdulist_sky[k]:
                if '_' in hdulist_sky[k][119:121]:
                    if int(hdulist_sky[k][119:120]) == origarm:
                        hdulist1_sky = fits.open(hdulist_sky[k])
                else:
                    if int(hdulist_sky[k][119:121]) == origarm:
                        hdulist1_sky = fits.open(hdulist_sky[k])
            if 'CL0036_IZ' in hdulist[i] and 'CL0036_IZ' in hdulist_sky[k]:
                if '_' in hdulist_sky[k][119:121]:
                    if int(hdulist_sky[k][119:120]) == origarm:
                        hdulist1_sky = fits.open(hdulist_sky[k])
                else:
        ...
        for j in range(len(hdulist1[1].data)):
            ...
            sky_array = hdulist1_sky[1].data[j]
            sky_array = sky_array[~np.isnan(sky_array)]
            summed_sky_flux = np.sum(sky_array)
            ...
            sky_flux.append(summed_sky_flux)    #to be used in plotting spectrum for local sky
        ...
        sky_flux = sky_flux * np.median(median_flux)/np.median(sky_flux)
        if 'YJ' in hdulist[i] and not os.path.isfile('./figures/'+hdulist[i][62:-5]+'_Halpha.pdf'):
            ...
            NII_1 = 0.6548 * ( 1 + z )
            NII_2 = 0.6584 * ( 1 + z )
            line2, = plt.plot((l,l),y_range, label='H$\\alpha$ at %.4f um'%(l))
            line3, = plt.plot((NII_1,NII_1), y_range, label='NII at %.4f um'%(NII_1))
            line4, = plt.plot((NII_2,NII_2), y_range, label='NII at %.4f um'%(NII_2))
            ...
            plt.savefig('./figures/'+hdulist[i][62:-5]+'_Halpha.pdf', bbox_extra_artists=(lgd,), bbox_inches='tight')
            plt.close()
            hdulist1.close()
            hdulist1_sky.close()
        ...
        if 'IZ' in hdulist[i] and not os.path.isfile('./figures/'+hdulist[i][62:-5]+'_Hbeta.pdf') and not os.path.isfile('./figures/'+hdulist[i][62:-5]+'_OIII.pdf'):
            ...
            y_range = (minimum,maximum)
            Hbeta = 0.4861 * ( 1 + z )
            line2, = plt.plot((Hbeta,Hbeta), y_range, label='H$\\beta$ at %.4f um'%(Hbeta))
            line2.set_dashes([4,2,2,2])
            ...
            plt.savefig('./figures/'+hdulist[i][62:-5]+'_Hbeta.pdf', bbox_extra_artists=(lgd,), bbox_inches='tight')
            plt.close()
            ...
            y_range = (minimum,maximum)
            OIII = 0.5007 * ( 1 + z )
            line3, = plt.plot((OIII,OIII), y_range, label='OIII at %.4f um'%(OIII))
            line3.set_dashes([4,2,2,2])
            line5, = plt.plot((0.78,1.08985),(0.0,0.0))
            plt.title('Total Flux vs Wavelength centered on OIII from \n'+hdulist[i],fontsize=10)
            plt.xlabel('Wavelength (microns)',fontsize=10)
            plt.ylabel('Flux (ergs cm$\mathregular{^{-2}}$ $\\AA\mathregular{^{-1}}$ s$\mathregular{^{-1}}$)',fontsize=10)
            plt.axis('tight')
            ...
            plt.savefig('./figures/'+hdulist[i][62:-5]+'_OIII.pdf', bbox_extra_artists=(lgd,), bbox_inches='tight')
            plt.close()
            hdulist1.close()
            hdulist1_sky.close()
