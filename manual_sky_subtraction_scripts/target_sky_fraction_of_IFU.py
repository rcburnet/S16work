    ...
    for j in range(len(hdulist_sci[i])):
        ...
        for k in range(len(hdufits_sci)):   #loop into each ifu of the science/sky frame
            ...
            #plot spectrum for each arm now
                if not os.path.exists('../figures/laptop/target_sky_fraction_of_object/sky_subtracted_spectrum_of_IFUs/all_ifus/'):
                    os.makedirs('../figures/laptop/target_sky_fraction_of_object/sky_subtracted_spectrum_of_IFUs/all_ifus/')
                line1, = plt.plot(x,sky_subtracted_sum_data, label='Total Flux of Target IFU')
                line2, = plt.plot(x, avg_flux_sky_list, label='Mean Flux of Scaled Sky IFU')
                ...
                print '../figures/laptop/target_sky_fraction_of_object/sky_subtracted_spectrum_of_IFUs/all_ifus/'+hdulist_sci[i][j][-66:]+'_IFU_'+str(k)+'.pdf'
                plt.savefig('../figures/laptop/target_sky_fraction_of_object/sky_subtracted_spectrum_of_IFUs/all_ifus/'+hdulist_sci[i][j][-66:]+'_IFU_'+str(k)+'.pdf',bbox_extra_artists=(lgd,),bbox_inches='tight')
                plt.close()
        hdufits_sci.close()
        hdufits_sky.close()
