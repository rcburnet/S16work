    ...
    for j in range(len(hdulist_sci[i])):
        ...
        for k in k_list:
            ...
            #plot spectrum for each arm now
                if not os.path.exists('../figures/laptop/target_sky_fraction_of_object/sky_subtracted_spectrum_of_IFUs/specific_targets_specific_regions/'):
                    os.makedirs('../figures/laptop/target_sky_fraction_of_object/sky_subtracted_spectrum_of_IFUs/specific_targets_specific_regions/')
                if k == 8:
                    z = 0.869
                if k == 20:
                    z = 0.867
                ...
                print '../figures/laptop/target_sky_fraction_of_object/sky_subtracted_spectrum_of_IFUs/specific_targets_specific_regions/'+hdulist_sci[i][j][-66:-52]+'_'+hdulist_sci[i][j][-28:-5]+'_Target_'+str(target[k])+'_specific_region.pdf'
                plt.savefig('../figures/laptop/target_sky_fraction_of_object/sky_subtracted_spectrum_of_IFUs/specific_targets_specific_regions/'+hdulist_sci[i][j][-66:-52]+'_'+hdulist_sci[i][j][-28:-5]+'_Target_'+str(target[k])+'_specific_region.pdf',bbox_extra_artists=(lgd,),bbox_inches='tight')
                plt.close()
        hdufits_sci.close()
        hdufits_sky.close()
