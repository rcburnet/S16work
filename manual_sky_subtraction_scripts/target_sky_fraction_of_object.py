...
####
# script to calculate f = <T_lambda(target)>/<S_lambda(sky)> , the fraction of average flux of the target frame target exposures to the average flux of the associated sky frame sky exposures to see if they agree. Plots pdfs of filenames "fraction_*.pdf" in the directory ./figures/target_sky_subtraction_of_object/ . Then, it carries out sky subtraction of target cubes manually using T_lambda - S_lambda*f for each spaxel in the target cube and plots the spectrum of that in the same directory.
####
...
for i in range(len(f)):
    ...
    if not os.path.exists('../figures/laptop/target_sky_fraction_of_object/fraction/'):
        os.makedirs('../figures/laptop/target_sky_fraction_of_object/fraction/')
    ...
    print '../figures/laptop/target_sky_fraction_of_object/fraction/'+hdulist_sci[i][-48:]+'.pdf'
    plt.savefig('../figures/laptop/target_sky_fraction_of_object/fraction/'+hdulist_sci[i][-48:]+'.pdf')
    plt.close()
