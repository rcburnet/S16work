...
for i in range(len(hdulist_sci)):
   ...
   #     total_flux_sky = np.sum(hdufits_sky[1].data[j][~np.isnan(hdufits_sky[1].data[j])])
        sky_subtracted_sum_data[i].append(np.mean(spaxel_array[~np.isnan(spaxel_array)]))
...
for i in range(len(sky_subtracted_sum_data)):
    if 'CL0034-YJ-OB-1_COMBINE_SCI_RECONSTRUCTED_11' in hdulist_sci[i] or 'CL0034-YJ-OB-1_COMBINE_SCI_RECONSTRUCTED_5' in hdulist_sci[i] or 'CL0036-YJ-OB-1_COMBINE_SCI_RECONSTRUCTED_4' in hdulist_sci[i] or 'CL0036-YJ-OB-1_COMBINE_SCI_RECONSTRUCTED_15' in hdulist_sci[i]:
        ...
        if not os.path.exists('../figures/laptop/target_sky_fraction_of_object/sky_subtracted_spectrum_of_bright_object_0.05microns_mean/'):
            os.makedirs('../figures/laptop/target_sky_fraction_of_object/sky_subtracted_spectrum_of_bright_object_0.05microns_mean/')
        line1, = plt.plot(x,sky_subtracted_sum_data[i], label='Mean Flux of Target Slice')
        if 'RECONSTRUCTED_5' in hdulist_sci[i]:
            plt.title('Mean Flux vs Wavelength  from \n '+hdulist_sci[i][-47:], fontsize=10)
            z = 0.869
        ...
        NII_1 = 0.6548 * ( 1 + z )
        NII_2 = 0.6584 * ( 1 + z )
        y_range = (min(sky_subtracted_sum_data[i]),max(sky_subtracted_sum_data[i]))
        ...
        line6, = plt.plot(x,avg_flux_sky_list[i], label='Scaled Mean Sky Flux')
        plt.xlabel('Wavelength (microns)',fontsize=10)
        plt.ylabel('Mean Flux (ergs cm$\mathregular{^{-2}}$ $\\AA\mathregular{^{-1}}$ s$\mathregular{^{-1}}$)',fontsize=10)
        plt.axis('tight')
        ...
        print '../figures/laptop/target_sky_fraction_of_object/sky_subtracted_spectrum_of_bright_object_0.05microns_mean/'+hdulist_sci[i][-48:]+'.pdf'
        plt.savefig('../figures/laptop/target_sky_fraction_of_object/sky_subtracted_spectrum_of_bright_object_0.05microns_mean/'+hdulist_sci[i][-48:]+'.pdf',bbox_extra_artists=(lgd,),bbox_inches='tight')
        plt.close()
