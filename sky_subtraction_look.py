...
for j in range(len(hdulist)):
	...
	x = []					#wavelength array
        ...
        for i in range(len(hdulist1[1].data)):	#look at IFU1 data and loop through data array
		total_flux.append(np.sum(hdulist1[1].data[i][~np.isnan(hdulist1[1].data[i])]))	#sum all flux and append to temporary total flux array
...
for i in range(len(hdufits_of_obj_4[1].data)):	#loop into object fits file data
	total_flux.append(np.sum(hdufits_of_obj_4[1].data[i][~np.isnan(hdufits_of_obj_4[1].data[i])]))	#sum all flux and append to temporary total flux array
#	if i == 947:		#location of largest peak
#		print total_flux[i]	
...
#Plot
line1, = plt.plot(x,total_flux_tot[0],label='Sky 2014-07-11T06:34:52.380')
line6, = plt.plot(x,total_flux_tot[1],label='Sky 2014-07-11T06:50:52.037')
line7, = plt.plot(x,total_flux_tot[2],label='Sky 2014-07-11T06:59:01.745')
line8, = plt.plot(x,total_flux_tot[3],label='Object')
plt.title('Total Flux vs Wavelength of Skies and Object of Target 4 (IFU 1)',fontsize=10)
...
plt.setp(line7, linewidth=0.1, color='g')
plt.setp(line8, linewidth=0.1, color='y')
