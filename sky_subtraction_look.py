...
for j in range(len(hdufits_of_obj_4)):
	...
	for i in range(len(hdufits_obj[1].data)):	#loop into object fits file data
		...
		total_flux.append(np.sum(hdufits_obj[1].data[i][~np.isnan(hdufits_obj[1].data[i])]))      #sum all flux and append to temporary total flux array
		if i == 947:		#location of largest peak
			y.append(hdufits_obj[1].data[i])	
