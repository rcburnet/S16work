...
#####
# script that will plot total flux vs wavelength while ignoring NaN of combined sky subtracted and calibrated cubes (pipeline run) in /home/rburnet/reflex/project_data/OBs_combined/sky_subtracted/ except for sky arms
#####
...
for j in range(len(hdulist)):
    ...
    if not os.path.isfile('./figures/sky_subtracted_calibrated/'+hdulist[j][-43:]+'.pdf'):
        ...
        if not os.path.exists('./figures/sky_subtracted_calibrated/'+hdulist[j][-43:-33]):
            os.makedirs('./figures/sky_subtracted_calibrated/'+hdulist[j][-43:-33])
        ...
        print './figures/sky_subtracted_calibrated/'+hdulist[j][-43:]+'.pdf'
        plt.savefig('./figures/sky_subtracted_calibrated/'+hdulist[j][-43:]+'.pdf')
        plt.close()
        hdulist1.close()
